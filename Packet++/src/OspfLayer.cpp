...
void HelloContent::Print(std::ostream &os)
{
	os << "HelloContent:" << '\n';
	os << "mask: " << num2ip(mask) << '\n';
	os << "designated router: " << num2ip(dr) << '\n';
	os << "backup designated router: " << num2ip(br) << '\n';
	for (auto &var : neighbors)
	{
		os << "active neighbor: " << num2ip(var) << '\n';
	}
}
...
void RouterLsaContent::Print(std::ostream &os)
{
	...
	{
		os << "p2p link:" << '\n';
		os << "link id(neighbor): " << num2ip(link_id) << '\n';
		os << "link data(interface num): " << link_data << '\n';
	}
	else if (link_type == 2) //穿越
	{
		os << "transit link:" << '\n';
		os << "link id(designated router): " << num2ip(link_id) << '\n';
		os << "link(router ip): " << num2ip(link_data) << '\n';
	}
	else if (link_type == 3) //残桩
	{
		os << "stub link:" << '\n';
		os << "link id(network ip): " << num2ip(link_id) << '\n';
		os << "link data(network mask): " << num2ip(link_data) << '\n';
	}
	else if (link_type == 4) //虚拟
	{
		os << "virtual link:" << '\n';
		os << "link id(neighbor): " << num2ip(link_id) << '\n';
		os << "link data(router ip): " << num2ip(link_data) << '\n';
	}

	os << "link type: " << (uint16_t)link_type << '\n';
	os << "number of tos: " << (uint16_t)tos_num << '\n';
	os << "tos0 metric: " << tos0_metric << '\n';
	for (auto &var : toses)
	{
		os << "tos -- metric: " << var[0] << "--" << var[1] << '\n';
	}
}
...
{
	os << "RouterLsa:" << '\n';
	os << "lsa type: " << (uint16_t)lsa_type << '\n';
	os << "link state id: " << num2ip(state_id) << '\n';
	os << "advertising router: " << num2ip(adv_router) << '\n';
	os << "total length: " << tlen << '\n';
	os << "number of links: " << link_num << '\n';
...
void NetworkLsa::Print(std::ostream &os)
{
	...
	os << "advertising router: " << num2ip(adv_router) << '\n';
	os << "total length: " << tlen << '\n';
	os << "netmask: " << num2ip(mask) << '\n';
	for (auto &var : attach_routers)
	{
		os << "attach_router: " << var << '\n';
	}
}
...
{
	os << "PROTOCOLTYPE: OSPF" << '\n';
	os << "total length: " << getDataLen() << '\n';
	os << "source router: " << num2ip(getSrcRouter()) << '\n';
	os << "area id: " << num2ip(getArea()) << '\n';
