 ...
 * - IPReassembly#processPacket() - process a fragment. This is the main method which should be called whenever a new
 * fragment arrives. This method processes the fragment, runs the reassembly logic and returns the result packet when
 * it's fully reassembled
 * - IPReassembly#getCurrentPacket() - get the reassembled data that is currently available, even if reassembly process
 * is not yet completed
 * - IPReassembly#removePacket() - remove all data that is currently stored for a packet, including the reassembled data
...
class IPReassembly
{
  public:
	...
	/**
	 * @class IPv6PacketKey
	 * Represents a key that can uniquely identify IPv6 fragment packets. The key comprises of source IPv6 address, dest
	 * IPv6 address and fragment ID (which resides in the IPv6 fragmentation extension)
	 */
	 ...
	 * determined in its c'tor (default value is #PCPP_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE). When traffic volume
	 * exceeds this capacity the mechanism starts dropping packets in a LRU manner (least recently used are dropped
	 * first). Whenever a packet is dropped this callback is fired
	 ...
	 */
	IPReassembly(OnFragmentsClean onFragmentsCleanCallback = NULL, void *callbackUserCookie = NULL,
	 ...
	 * - The input fragment came out-of-order, meaning that wasn't the fragment that was currently expected (it's data
	 * is copied to the out-of-order fragment list)
	 * - The input fragment is malformed and will be ignored
	 * - The input fragment is the last one and the packet is now fully reassembled. In this case the return value will
	 * contain a pointer to the reassebmled packet
	 ...
	 * performance impact and memory consumption of parsing the whole packet. Default value is ::UnknownProtocol which
	 * means don't take this parameter into account
	 * @param[in] parseUntilLayer Optional parameter. Parse the reassembled packet until you reach a certain layer in
	 * the OSI model (inclusive). Can be useful for cases when you need to parse only up to a certain OSI layer (for
	 * example transport layer) and want to avoid the performance impact and memory consumption of parsing the whole
	 * packet. Default value is ::OsiModelLayerUnknown which means don't take this parameter into account
	 * @return
	 ...
	 * - If the input fragment is the last one and the reassembled packet is ready - a pointer to the reassembled packet
	 * is returned. Notice it's the user's responsibility to free this pointer when done using it
	 ...
	 * protocol (inclusive). Can be useful for cases when you need to parse only up to a certain layer and want to avoid
	 * the performance impact and memory consumption of parsing the whole packet. Default value is ::UnknownProtocol
	 * which means don't take this parameter into account
	 * @param[in] parseUntilLayer Optional parameter. Parse the raw and reassembled packets until you reach a certain
	 * layer in the OSI model (inclusive). Can be useful for cases when you need to parse only up to a certain OSI layer
	...
	/**
	 * Get a partially reassembled packet. This method returns all the reassembled data that was gathered so far which
	 * is obviously not a fully reassembled packet (otherwise it would have returned by processPacket()). Notice all
	 * data is being copied so the user is responsible to free the returned Packet object when done using it. Notice#2 -
	 * calling this method doesn't interfere with the reassembly of this packet - all internal structures and data
	 * remain
	 * @param[in] key The identifiers of the packet to return
	 * @return A pointer to a Packet object containing the partially reassembled packet. Notice the user is responsible
	 * to free this object when done using it
	 */
	Packet *getCurrentPacket(const PacketKey &key);

	/**
	 * Remove a partially reassembled packet from all internal structures. That means that if another fragment of this
	 * packet appears it will be treated as a new packet
