...
namespace pcpp
{
	...
	enum LinkLayerType
	{
		...
		/** PPP in HDLC-like encapsulation, like LINKTYPE_PPP_HDLC, but with the 0xff address byte replaced by a direction
		 * indication - 0x00 for incoming and 0x01 for outgoing */
		...
		/** Link Access Procedures on the D Channel (LAPD) frames */
		LINKTYPE_LAPD = 203,
		/** PPP, as per RFC 1661 and RFC 1662, preceded with a one-byte pseudo-header with a zero value meaning "received by
		 * this host" and a non-zero value meaning  "sent by this host" */
		...
		LINKTYPE_DBUS = 231,
		/** DVB-CI (DVB Common Interface for communication between a PC Card module and a DVB receiver), with the message
		...
		/** D_PDUs as described by NATO standard STANAG 5066, starting with the synchronization sequence, and including both
		 * header and data CRCs */
		LINKTYPE_STANAG_5066_D_PDU = 237,
		/** Linux netlink NETLINK NFLOG socket log messages */
		LINKTYPE_NFLOG = 239,
		/** Pseudo-header for Hilscher Gesellschaft für Systemautomation mbH netANALYZER devices, followed by an Ethernet
		 * frame, beginning with the MAC header and ending with the FCS */
		LINKTYPE_NETANALYZER = 240,
		/** Pseudo-header for Hilscher Gesellschaft für Systemautomation mbH netANALYZER devices, followed by an Ethernet
		 * frame, beginning with the preamble, SFD, and MAC header, and ending with the FCS */
		LINKTYPE_NETANALYZER_TRANSPARENT = 241,
		/** IP-over-InfiniBand, as specified by RFC 4391 section 6 */
		LINKTYPE_IPOIB = 242,
		/** MPEG-2 Transport Stream transport packets, as specified by ISO 13818-1/ITU-T Recommendation H.222.0 */
		LINKTYPE_MPEG_2_TS = 243,
		/** Pseudo-header for ng4T GmbH's UMTS Iub/Iur-over-ATM and Iub/Iur-over-IP format as used by their ng40 protocol
		 * tester */
		LINKTYPE_NG40 = 244,
		 ...
		 * (Proximity Coupling Device, PCD), with the message format specified by the PCAP format for ISO14443 specification
		 */
...
#define PCPP_MAX_PACKET_SIZE 65536
	...
	public:
		 ...
		 * capturing engine allocates the raw data memory and give the user a pointer to it + a timestamp it has arrived to
		 * the device
		 ...
		 * @param[in] deleteRawDataAtDestructor An indicator whether raw data pointer should be freed when the instance is
		 * freed or not. If set to 'true' than pRawData will be freed when instanced is being freed
		...
		/**
		 * A copy constructor that copies all data from another instance. Notice all raw data is copied (using memcpy), so
		 * when the original or the other instance are freed, the other won't be affected
		 ...
		 * instance, the original raw data is freed first. Then the other instance is copied to this instance, the same way
		 ...
		 * @param[in] frameLength When reading from pcap files, sometimes the captured length is different from the actual
		 * packet length. This parameter represents the packet length. This parameter is optional, if not set or set to -1
		 * it is assumed both lengths are equal
		 ...
		 * @return True if raw data was set for this instance. Raw data can be set using the non-default constructor, using
		 * setRawData(), using the copy constructor or using the assignment operator. Returns false otherwise, for example:
		 * if the instance was created using the default constructor or clear() was called
		 */
		...
		/**
		 * Clears all members of this instance, meaning setting raw data to NULL, raw data length to 0, etc. Currently raw
		 * data is always freed, even if deleteRawDataAtDestructor was set to 'false'
		...
		/**
		 * Append data to the end of current data. This method works without allocating more memory, it just uses memcpy()
		 * to copy dataToAppend at the end of the current data. This means that the method assumes this memory was already
		 * allocated by the user. If it isn't the case then this method will cause memory corruption
		...
		/**
		 * Insert new data at some index of the current data and shift the remaining old data to the end. This method works
		 * without allocating more memory, it just copies dataToAppend at the relevant index and shifts the remaining data
		 * to the end. This means that the method assumes this memory was already allocated by the user. If it isn't the
		 * case then this method will cause memory corruption
		 ...
		 * @return True if all bytes were removed successfully, or false if atIndex+numOfBytesToRemove is out-of-bounds of
		 * the raw data buffer
		 */
		 ...
		 * allocated buffer to the new one. This method can become useful if the user wants to insert or append data to the
		 * raw data, and the previous allocated buffer is too small, so the user wants to allocate a larger buffer and get
		 * RawPacket instance to point to it
		 * @param[in] newBufferLength The new buffer length as required by the user. The method is responsible to allocate
		 * the memory
