...
/**
 * @class GreLayer
 * Abstract base class for GRE layers (GREv0Layer and GREv1Layer). Cannot be instantiated and contains common logic for
 * derived classes
 */
...
class GREv0Layer : public GreLayer
{
  public:
	 ...
	 * gre_basic_header#routingBit, gre_basic_header#checksumBit, gre_basic_header#ackSequenceNumBit) without using the
	 * proper set or unset methods (such as setChecksum(), unsetChecksum(), etc.) may result to wrong calculation of
	 * header length and really weird bugs. Please avoid doing so
	 ...
	 * remain unset), the new checksum value will be set and offset value will be set to 0. The reason both fields are
	 * added is that GREv0 protocol states both of them or none of them should exist on packet (even if only one of the
	 * bits are set)
	 ...
	 * gre_basic_header#routingBit is not set as well. Otherwise checksum field will remain on packet with value of 0
...
class GREv1Layer : public GreLayer
{
  public:
	...
	/**
	 * Get a pointer to the basic GREv1 header containing all non-optional fields. Notice this points directly to the
	 * data, so every change will change the actual packet data. Also please notice that changing the set bits
	 * (gre_basic_header#strictSourceRouteBit, gre_basic_header#sequenceNumBit, gre_basic_header#keyBit,
	 * gre_basic_header#routingBit, gre_basic_header#checksumBit, gre_basic_header#ackSequenceNumBit) without using the
	 * proper set or unset methods (such as setAcknowledgmentNum(), unsetSequenceNumber(), etc.) may result to wrong
	 * calculation of header length or illegal GREv1 packet and to some really weird bugs. Please avoid doing so
...
class PPP_PPTPLayer : public Layer
{
  public:
	...
	/**
	 * Get a pointer to the PPP-PPTP header. Notice this points directly to the data, so every change will change the
	 * actual packet data
