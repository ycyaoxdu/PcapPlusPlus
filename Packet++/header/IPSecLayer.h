...
class AuthenticationHeaderLayer : public Layer
{
  public:
	...
	/**
	 * Currently identifies the following next layers: UdpLayer, TcpLayer, IPv4Layer, IPv6Layer and ESPLayer. Otherwise
	 * sets PayloadLayer
	 */
	void parseNextLayer();
...
class ESPLayer : public Layer
{
  public:
	 ...
	 * @param[in] packet A pointer to the Packet instance where layer will be stored in
	 */
	ESPLayer(uint8_t *data, size_t dataLen, Layer *prevLayer, Packet *packet) : Layer(data, dataLen, prevLayer, packet)
	{
		m_Protocol = ESP;
	}
