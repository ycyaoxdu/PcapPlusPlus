...
class IPv4Option : public TLVRecord<uint8_t, uint8_t>
{
  public:
	...
	/**
	 * A method for parsing the IPv4 option value as a list of IPv4 addresses. This method is relevant only for certain
	 * types of IPv4 options which their value is a list of IPv4 addresses such as ::IPV4OPT_RecordRoute,
	 * ::IPV4OPT_StrictSourceRoute, ::IPV4OPT_LooseSourceRoute, etc. This method returns a vector of the IPv4 addresses.
	 * Blank IP addresses (meaning zeroed addresses - 0.0.0.0) will not be added to the returned list. If some error
	 * occurs during the parsing or the value is invalid an empty vector is returned
	...
	/**
	 * A method for parsing the IPv4 timestamp option value. This method is relevant only for IPv4 timestamp option. For
	 * other option types an empty result will be returned. The returned structure contains the timestamp value type
	 * (timestamp only, timestamp + IP addresses, etc.) as well as 2 vectors containing the list of timestamps and the
	 * list of IP addresses (if applicable for the timestamp value type). Blank timestamps or IP addresses (meaning
	 * zeroed values - timestamp=0 or IP address=0.0.0.0) will not be added to the lists. If some error occurs during
	 * the parsing or the value is invalid an empty result is returned
...
class IPv4OptionBuilder : public TLVRecordBuilder
{
  ...
  public:
	 ...
	 * way. For option types ::IPV4OPT_NOP and ::IPV4OPT_EndOfOptionsList this parameter is ignored (expected to be
	 * NULL) as these option types don't contain any data
	...
	/**
	 * A c'tor for building IPv4 timestamp option (::IPV4OPT_Timestamp). The IPv4Option object can be later retrieved by
	 * calling build()
...
class IPv4Layer : public Layer, public IPLayer
{
  public:
	 ...
	 * @param[in] prevLayer A pointer to the previous layer
	 * @param[in] packet A pointer to the Packet instance where layer will be stored in
	 * @param[in] setTotalLenAsDataLen When setting this value to "true" or when using the other c'tor, the layer data
	 * length is calculated from iphdr#totalLength field. When setting to "false" the data length is set as the value of
	 * dataLen parameter. Please notice that if iphdr#totalLength is equal to zero (which can happen in TCP Segmentation
	...
	/**
	 * Get a pointer to the IPv4 header. Notice this points directly to the data, so every change will change the actual
	 * packet data
	 ...
	 * parameter which gets a default value of ::IPV4OPT_Unknown if omitted, which means the new option will be added as
	 * the first option in the layer
