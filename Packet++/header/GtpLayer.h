  ...
  public:
	...
	class GtpExtension
	{
	  ...
	  public:
		...
		/**
		 * @return The extension type of the next header. If there are no more header extensions or if this object is
		 * empty a value of zero is returned
		 */
		uint8_t getNextExtensionHeaderType() const;

		/**
		 * @return An instance of this class representing the next extension header, if exists in the message. If there
		 * are no more header extensions or if this object is empty an empty instance of GtpExtension is returned,
		 * meaning one that GtpExtension#isNull() returns "true"
		 */
	 ...
	 * @param[in] seqNum The sequence number to be set in the newly created later. If "setSeqNum" is set to false this
	 ...
	 * @param[in] npduNum The N-PDU number to be set in the newly created later. If "setNpduNum" is set to false this
	 * parameter will be ignored
	 */
	...
	/**
	 * @return The GTP v1 common header structure. Notice this points directly to the data, so every change will change
	 ...
	 * @return True if the value was set successfully, false otherwise. In case of failure a corresponding error message
	...
	/**
	 * Add a GTPv1 header extension. It is assumed that the extension is 4 bytes in length and its content is 2 bytes in
	 * length. If you need a different content size please reach out to me. This method takes care of extending the
	 * layer to make room for the new extension and also sets the relevant flags and fields
	 ...
	 * object will be returned (meaning GtpExtension#isNull() will return "true") and a corresponding error message will
	 * be written to log
	 */
