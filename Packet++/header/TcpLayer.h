...
class TcpOptionBuilder : public TLVRecordBuilder
{
  ...
  public:
	 ...
	 */
	TcpOptionBuilder(TcpOptionType optionType, uint8_t optionValue) : TLVRecordBuilder((uint8_t)optionType, optionValue)
	{
	}
...
class TcpLayer : public Layer
{
  public:
	...
	/**
	 * Get a pointer to the TCP header. Notice this points directly to the data, so every change will change the actual
	 * packet data
	 ...
	 * @param[in] prevOptionType The TCP option which the newly added option should come after. This is an optional
	 * parameter which gets a default value of ::TCPOPT_Unknown if omitted, which means the new option will be added as
	 ...
	 * @return True if TCP option was removed or false if type wasn't found or if removal failed (in each case a proper
