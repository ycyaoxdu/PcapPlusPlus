...
class PayloadLayer : public Layer, public ParsedResult
{
  public:
	 ...
	 * @param[in] dummy A dummy parameter to separate the constructor signature from the other constructor. Its value
	 * isn't used anywhere
	 ...
	 * 0001080006040002842b2b774c56c0a80078000000000000c0a8. In order for the hex stream to be valid it has to contain
	 * valid hex chars only (which means, for example, that it can't begin with "0x") and it also has to have an even
	 * number of chars (each char represents one nibble). If the string is not a valid hex stream an error will be
	 * printed to log and the payload layer will be empty (no data)
	 */
