 ...
 * An interface (virtual abstract class) that indicates an object that holds a pointer to a buffer data. The Layer class
 * is an example of such object, hence it inherits this interface
 */
class IDataContainer
{
  public:
...
/**
 * @class Layer
 * Layer is the base class for all protocol layers. Each protocol supported in PcapPlusPlus has a class that inherits
 Layer.
 ...
 * Each protocol instance is obviously part of a protocol stack (which construct a packet). This protocol stack is
 represented
 * in PcapPlusPlus in a linked list, and each layer is an element in this list. That's why each layer has proprties to
 ...
 * The Layer class, as a base class, is abstract and the user can't create an instance of it (it has a private
 constructor)
 * Each layer holds a pointer to the relevant place in the packet. The layer sees all the data from this pointer forward
 until the
...
class Layer : public IDataContainer
{
  ...
  public:
	 ...
	 * 1. from an existing packet - this is the case when parsing packets received from files or the network. In this
	 * case the data was already allocated by someone else, and layer only holds the pointer to the relevant place
	 * inside this data
	 * 2. when creating packets, data is allocated when layer is created. In this case the layer is responsible for
	 * freeing it as well
	 *
	 * @return Returns true if the data was allocated by an external source (a packet) or false if it was allocated by
	 * the layer itself
	 */
  ...
  protected:
	...
	Layer()
		: m_LayerNumber(0),m_Data(NULL), m_DataLen(0), m_Packet(NULL), m_Protocol(UnknownProtocol), m_NextLayer(NULL), m_PrevLayer(NULL),
		  m_IsAllocatedInPacket(false)
	{
	}

	Layer(uint8_t *data, size_t dataLen, Layer *prevLayer, Packet *packet)
		: m_LayerNumber(0),m_Data(data), m_DataLen(dataLen), m_Packet(packet), m_Protocol(UnknownProtocol), m_NextLayer(NULL),
