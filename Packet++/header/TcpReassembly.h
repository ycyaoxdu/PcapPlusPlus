...
/**
 * @class TcpStreamData
 * When following a TCP connection each packet may contain a piece of the data transferred between the client and the
 * server. This class represents these pieces: each instance of it contains a piece of data, usually extracted from a
 * single packet, as well as information about the connection
 */
class TcpStreamData
{
  public:
...
struct TcpReassemblyConfiguration
{
	...
	/** How long the closed connections will not be cleaned up. The value is expressed in seconds. If the value is set
	 * to 0 then TcpReassembly should use the default value. This parameter is only relevant if removeConnInfo is equal
	 * to true.
	 */
	uint32_t closedConnectionDelay;

	/** The maximum number of items to be cleaned up per one call of purgeClosedConnections. If the value is set to 0
	 * then TcpReassembly should use the default value. This parameter is only relevant if removeConnInfo is equal to
	 * true.
	 */
	uint32_t maxNumToClean;

	/** The maximum number of fragments with a non-matching sequence-number to store per connection flow before packets
	   are assumed permanently missed. If the value is 0, TcpReassembly should keep out of order fragments indefinitely,
	   or until a message from the paired side is seen.
	 */
	 ...
	 * @param[in] removeConnInfo The flag indicating whether to remove the connection data after a connection is closed.
	 * The default is true
	 * @param[in] closedConnectionDelay How long the closed connections will not be cleaned up. The value is expressed
	 * in seconds. If it's set to 0 the default value will be used. The default is 5.
	 * @param[in] maxNumToClean The maximum number of items to be cleaned up per one call of purgeClosedConnections. If
	 * it's set to 0 the default value will be used. The default is 30.
	 ...
	 * @param[in] enableBaseBufferClearCondition To enable to clear buffer once packet contains data from a different
...
class TcpReassembly
{
  public:
	...
	enum ReassemblyStatus
	{
		 ...
		 *    Note if there's any buffered out-of-order packet waiting for this packet, their associated callbacks are
		 * called in this `reassemblePacket` call.
		 * 2. An out-of-order TCP packet which satisfy `packet_sequence < sequence_expected && packet_sequence +
		 * packet_payload_length > sequence_expected`. Note only the new data (the `[sequence_expected, packet_sequence
		 * + packet_payload_length]` part ) is processed by `OnMessageReadyCallback` callback function.
		 */
		TcpMessageHandled,
		/**
		 * The processed packet is an out-of-order TCP packet, meaning `packet_sequence > sequence_expected`. It's
		 * buffered so no `OnMessageReadyCallback` callback function is called. The callback function for this packet
		 * maybe called LATER, under different circumstances:
		 ...
		 *    If this packet contains "new data", meaning `(packet_sequence <= sequence_expected) && (packet_sequence +
		 * packet_payload_length > sequence_expected)`, the new data is processed by `OnMessageReadyCallback` callback.
		 */
	 ...
	 * @param[in] side The side this data belongs to (MachineA->MachineB or vice versa). The value is 0 or 1 where 0 is
	 * the first side seen in the connection and 1 is the second side seen
	 * @param[in] tcpData The TCP data itself + connection information
	 * @param[in] userCookie A pointer to the cookie provided by the user in TcpReassembly c'tor (or NULL if no cookie
	 * provided)
	 ...
	 * @param[in] userCookie A pointer to an object provided by the user. This pointer will be returned when invoking
	 * the various callbacks. This parameter is optional, default cookie is NULL
	 * @param[in] onConnectionStartCallback The callback to be invoked when a new connection is identified. This
	 * parameter is optional
	 * @param[in] onConnectionEndCallback The callback to be invoked when a new connection is terminated (either by a
	 * FIN/RST packet or manually by the user). This parameter is optional
	...
	/**
	 * The most important method of this class which gets a packet from the user and processes it. If this packet opens
	 * a new connection, ends a connection or contains new data on an existing connection, the relevant callback will be
	 * called (TcpReassembly#OnTcpMessageReady, TcpReassembly#OnTcpConnectionStart, TcpReassembly#OnTcpConnectionEnd)
	 ...
	 */
	ReassemblyStatus reassemblePacket(Packet *tcpData, Layer *nextlayer = NULL, IPAddress *IpSrc = NULL,
									  IPAddress *IpDst = NULL);

	/**
	 * The most important method of this class which gets a raw packet from the user and processes it. If this packet
	 * opens a new connection, ends a connection or contains new data on an existing connection, the relevant callback
	 * will be invoked (TcpReassembly#OnTcpMessageReady, TcpReassembly#OnTcpConnectionStart,
	...
	/**
	 * Close all open connections manually. This method will cause the TcpReassembly#OnTcpConnectionEnd to be invoked
	 * for each connection with a reason of TcpReassembly#TcpReassemblyConnectionClosedManually
	 */
	 ...
	 * @return A positive number (> 0) if connection is opened, zero (0) if connection is closed, and a negative number
	 * (< 0) if this connection isn't managed by this TcpReassembly instance
	 */
