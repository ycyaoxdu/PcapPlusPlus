...
class IPv6Layer : public Layer, public IPLayer, public ParsedResult
{
  public:
	...
	/**
	 * Get a pointer to the IPv6 header. Notice this points directly to the data, so every change will change the actual
	 * packet data
	...
	/**
	 * A templated getter for an IPv6 extension of a type TIPv6Extension. TIPv6Extension has to be one of the supported
	 * IPv6 extensions, meaning a class that inherits IPv6Extension. If the requested extension type isn't found NULL is
	 * returned
	...
	/**
	 * Add a new extension of type TIPv6Extension to the layer. This is a templated method and TIPv6Extension has to be
	 * one of the supported IPv6 extensions, meaning a class that inherits IPv6Extension. If the extension is added
	 * successfully a pointer to the newly added extension object is returned, otherwise NULL is returned
	 * @param[in] extensionHeader The extension object to add. Notice the object passed here is read-only, meaning its
	 * data is copied but the object itself isn't modified
