 ...
 * The functionality that is added for this class is the HTTP first line concept. An HTTP request has the following
 * first line: <i>GET /bla/blabla.asp HTTP/1.1</i> Since it's not an "ordinary" HTTP field, it requires a special
 * treatment and gets a class of it's own: HttpRequestFirstLine. Unlike most L2-4 protocols, an HTTP request header can
 * spread over more than 1 packet. PcapPlusPlus currently doesn't support a header that is spread over more than 1
 * packet so in such cases: 1) only the first packet will be parsed as HttpRequestLayer (the other packets won't be
 * recognized as HttpRequestLayer) and 2) the HTTP header for the first packet won't be complete (as it continues in the
 * following packets), this why PcapPlusPlus can indicate that HTTP request header is complete or not(doesn't end with
...
class HttpRequestLayer : public HttpMessage
{
  ...
  public:
	...
	/**
	 * A constructor that allocates a new HTTP request header with only the first line filled. Object will be created
	 * without further fields. The user can then add fields using addField() methods
	...
	/**
	 * A copy constructor for this layer. This copy constructor inherits base copy constructor HttpMessage#HttpMessage()
	 * and add the functionality of copying the first line as well
	 ...
	 * request packet: URI is in the first line and hostname is in "HOST" field. This methods concatenates the hostname
	 * and URI to the full URL
 ...
 * The functionality that is added for this class is the HTTP first line concept. An HTTP response has the following
 * first line: <i>200 OK HTTP/1.1</i> Since it's not an "ordinary" HTTP field, it requires a special treatment and gets
 * a class of it's own: HttpResponseFirstLine. Unlike most L2-4 protocols, an HTTP response header can spread over more
 * than 1 packet. PcapPlusPlus currently doesn't support a header that is spread over more than 1 packet so in such
 * cases: 1) only the first packet will be parsed as HttpResponseLayer (the other packets won't be recognized as
 * HttpResponseLayer) and 2) the HTTP header for the first packet won't be complete (as it continues in the following
...
class HttpResponseLayer : public HttpMessage
{
  ...
  public:
	...
	/**
	 * A constructor that allocates a new HTTP response header with only the first line filled. Object will be created
	 * without further fields. The user can then add fields using addField() methods
	 * @param[in] version HTTP version to be used
	 * @param[in] statusCode Status code to be used
	 * @param[in] statusCodeString Most status codes have their default string, e.g 200 is usually "OK", 404 is usually
	 * "Not Found", etc. But the user can set a non-default status code string and it will be written in the header
	 * first line. Empty string ("") means using the default status code string
	 */
	...
	/**
	 * A copy constructor for this layer. This copy constructor inherits base copy constructor HttpMessage#HttpMessage()
	 * and adds the functionality of copying the first line as well
	 * @param[in] other The instance to copy from
	 */
	 ...
	 * - If the "Content-Length" field doesn't exist - the method will create this field and put the value in it. Here
	 * are also 2 cases:
	...
	/**
	 * The length of the body of many HTTP response messages is determined by a HTTP header field called
	 * "Content-Length". This method parses this field, extracts its value and return it. If this field doesn't exist
 ...
 * etc.), URI (e.g /main/index.html) and HTTP version (e.g HTTP/1.1). All these parameters are included in this class,
 * and the user can retrieve or set them. This class cannot be instantiated by users, it's created inside
 * HttpRequestLayer and user can get a pointer to an instance of it. All "get" methods of this class will retrieve the
 * actual data of the HTTP request and the "set" methods will change the packet data. Since HTTP is a textual protocol,
 * most fields aren't of fixed size and this also applies to the first line parameters. So most "set" methods of this
 * class need in most cases to shorten or extend the data in HttpRequestLayer. These methods will return a false value
...
class HttpRequestFirstLine
{
  ...
  public:
	 ...
	 * @return False if newMethod is HttpRequestLayer#HttpMethodUnknown or if shortening/extending the HttpRequestLayer
	...
	/**
	 * As explained in HttpRequestLayer, an HTTP header can spread over more than 1 packet, so when looking at a single
	 * packet the header can be partial. Same goes for the first line - it can spread over more than 1 packet. This
	 * method returns an indication whether the first line is partial
	 ...
	 * construction happens only in HttpRequestLayer). This kind of exception will be thrown if trying to construct with
	 * HTTP method of HttpRequestLayer#HttpMethodUnknown or with undefined HTTP version ::HttpVersionUnknown
	 */
...
/**
 * @class HttpResponseFirstLine
 * Represents an HTTP response header first line. The first line includes 2 parameters: status code (e.g 200 OK, 404 Not
 * Found, etc.), and HTTP version (e.g HTTP/1.1). These 2 parameters are included in this class, and the user can
 * retrieve or set them. This class cannot be instantiated by users, it's created inside HttpResponseLayer and user can
 * get a pointer to an instance of it. The "get" methods of this class will retrieve the actual data of the HTTP
 * response and the "set" methods will change the packet data. Since HTTP is a textual protocol, most fields aren't of
 * fixed size and this also applies to the first line parameters. So most "set" methods of this class need in most cases
 * to shorten or extend the data in HttpResponseLayer. These methods will return a false value if this action failed
 */
class HttpResponseFirstLine
{
  ...
  public:
	 ...
	 * @param[in] statusCodeString An optional parameter: set a non-default status code message (e.g "Bla Bla" instead
	 * of "Not Found"). If this parameter isn't supplied or supplied as empty string (""), the default message for the
	...
	/**
	 * As explained in HttpResponseLayer, an HTTP header can spread over more than 1 packet, so when looking at a single
	 * packet the header can be partial. Same goes for the first line - it can spread over more than 1 packet. This
	 * method returns an indication whether the first line is partial
	 * @return False if the first line is partial, true if it's complete
	 */
	 ...
	 * with HTTP status code of HttpResponseLayer#HttpStatusCodeUnknown or with undefined HTTP version
	 * ::HttpVersionUnknown
	 */
  ...
  private:
	...
	HttpResponseFirstLine(HttpResponseLayer *httpResponse, HttpVersion version,
						  HttpResponseLayer::HttpResponseStatusCode statusCode, std::string statusCodeString = "");

	static HttpVersion parseVersion(char *data, size_t dataLen);
	static HttpResponseLayer::HttpResponseStatusCode validateStatusCode(
