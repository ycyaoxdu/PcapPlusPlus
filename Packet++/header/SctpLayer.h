...
class SctpLayer : public Layer
{
  public:
	 ...
	 */
	SctpLayer(uint8_t *data, size_t dataLen, Layer *prevLayer, Packet *packet) : Layer(data, dataLen, prevLayer, packet)
	{
		m_Protocol = SCTP;
	}
	...
	/**
	 * Get a pointer to the SCTP header. Notice this points directly to the data, so every change will change the actual
	 * packet data
	 * @return A pointer to the @ref sctphdr
	 */
	sctphdr *getSctpHeader() const
	{
		return (sctphdr *)m_Data;
	}
