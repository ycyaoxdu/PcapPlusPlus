...
namespace pcpp
{
	...
	/**
	 * @class Packet
	 * This class represents a parsed packet. It contains the raw data (RawPacket instance), and a linked list of layers,
	 * each layer is a parsed protocol that this packet contains. The layers linked list is ordered where the first layer is
	 * the lowest in the packet (currently it's always Ethernet protocol as PcapPlusPlus supports only Ethernet packets),
	 * the next layer will be L2.5 or L3 (e.g VLAN, IPv4, IPv6, etc.), and so on. etc.), etc. The last layer in the linked
	 * list will be the highest in the packet. For example: for a standard HTTP request packet the layer will look like
	...
	public:
		 ...
		 * When using this constructor an empty raw buffer is allocated (with the size of maxPacketLen) and a new RawPacket
		 * is created
		 ...
		 * Instead, all of the raw data of this packet it written to this buffer: whenever a layer is added, it's data is
		 * written to this buffer. The buffer isn't freed and it's content isn't erased when the packet object is deleted.
		 ...
		 * came from the network. When using this constructor a pointer to the RawPacket is saved (data isn't copied) and
		 * the RawPacket is parsed, meaning all layers are created and linked to each other in the right order. In this
		 * overload of the constructor the user can specify whether to free the instance of raw packet when the Packet is
		 * free or not
		 * @param[in] rawPacket A pointer to the raw packet
		 * @param[in] freeRawPacket Optional parameter. A flag indicating if the destructor should also call the raw packet
		 * destructor or not. Default value is false
		 * @param[in] parseUntil Optional parameter. Parse the packet until you reach a certain protocol (inclusive). Can be
		 * useful for cases when you need to parse only up to a certain layer and want to avoid the performance impact and
		 * memory consumption of parsing the whole packet. Default value is ::UnknownProtocol which means don't take this
		 * parameter into account
		 * @param[in] parseUntilLayer Optional parameter. Parse the packet until you reach a certain layer in the OSI model
		 * (inclusive). Can be useful for cases when you need to parse only up to a certain OSI layer (for example transport
		 * layer) and want to avoid the performance impact and memory consumption of parsing the whole packet. Default value
		 * is ::OsiModelLayerUnknown which means don't take this parameter into account
		 */
		Packet(RawPacket *rawPacket, bool freeRawPacket = false, ProtocolType parseUntil = UnknownProtocol,
		 ...
		 * the RawPacket is parsed, meaning all layers are created and linked to each other in the right order. In this
		 * overload of the constructor the user can specify whether to free the instance of raw packet when the Packet is
		 * free or not. This constructor should be used to parse the packet up to a certain layer
		 * @param[in] rawPacket A pointer to the raw packet
		 * @param[in] parseUntil Optional parameter. Parse the packet until you reach a certain protocol (inclusive). Can be
		 * useful for cases when you need to parse only up to a certain layer and want to avoid the performance impact and
		 * memory consumption of parsing the whole packet
		 */
		 ...
		 * the RawPacket is parsed, meaning all layers are created and linked to each other in the right order. In this
		 * overload of the constructor the user can specify whether to free the instance of raw packet when the Packet is
		 * free or not. . This constructor should be used to parse the packet up to a certain layer in the OSI model
		 * @param[in] rawPacket A pointer to the raw packet
		 * @param[in] parseUntilLayer Optional parameter. Parse the packet until you reach a certain layer in the OSI model
		 * (inclusive). Can be useful for cases when you need to parse only up to a certain OSI layer (for example transport
		 * layer) and want to avoid the performance impact and memory consumption of parsing the whole packet
		 */
		 ...
		 * weren't allocated by this class, for example layers that were added by addLayer() or insertLayer() ). In addition
		 * it frees the raw packet if it was allocated by this instance (meaning if it was allocated by this instance
		 * constructor)
		 */
		virtual ~Packet()
		{
		...
		/**
		 * Assignment operator overloading. It first frees all layers allocated by this instance (Notice: it doesn't free
		 * layers that weren't allocated by this class, for example layers that were added by addLayer() or insertLayer() ).
		 * In addition it frees the raw packet if it was allocated by this instance (meaning if it was allocated by this
		 * instance constructor). Afterwards it copies the data from the other packet in the same way used in the copy
		 * constructor.
		 * @param[in] other The instance to copy from
		 */
		 ...
		 * @param[in] parseUntil Parse the packet until it reaches this protocol. Can be useful for cases when you need to
		 * parse only up to a certain layer and want to avoid the performance impact and memory consumption of parsing the
		 * whole packet. Default value is ::UnknownProtocol which means don't take this parameter into account
		 * @param[in] parseUntilLayer Parse the packet until certain layer in OSI model. Can be useful for cases when you
		 * need to parse only up to a certain layer and want to avoid the performance impact and memory consumption of
		 * parsing the whole packet. Default value is ::OsiModelLayerUnknown which means don't take this parameter into
		 * account
		 */
		void setRawPacket(RawPacket *rawPacket, bool freeRawPacket, ProtocolType parseUntil = UnknownProtocol,
						  OsiModelLayer parseUntilLayer = OsiModelLayerUnknown);
		 ...
		 * @param[in] ownInPacket If true, Packet fully owns newLayer, including memory deletion upon destruct.  Default is
		 * false.
		...
		/**
		 * Remove the first layer in the packet. The layer will be deleted if it was allocated during packet creation, or
		 * detached if was allocated outside of the packet. Please refer to removeLayer() to get more info
		 * @return True if layer removed successfully, or false if removing the layer failed or if there are no layers in
		 * the packet. In any case of failure an appropriate error log message will be printed
		 */
		 ...
		 * during packet creation or detached if were allocated outside of the packet, please refer to removeLayer() to get
		 * more info
		 * @param[in] layer A pointer to the layer to begin removing from. Please note this layer will not be removed, only
		 * the layers that come after it will be removed. Also, if removal of one layer failed, the method will return
		 * immediately and the following layers won't be deleted
		...
		/**
		 * Detach a layer from the packet. Detaching means the layer instance will not be deleted, but rather separated from
		 * the packet - e.g it will be removed from the layer chain of the packet and its data will be copied from the
		 * packet buffer into an internal layer buffer. After a layer is detached, it can be added into another packet (but
		 * it's impossible to attach a layer to multiple packets in the same time). After layer is detached, it's the user's
		 ...
		 * the packet - e.g it will be removed from the layer chain of the packet and its data will be copied from the
		 * packet buffer into an internal layer buffer. After a layer is detached, it can be added into another packet (but
		 * it's impossible to attach a layer to multiple packets at the same time). After layer is detached, it's the user's
		 * responsibility to delete it when it's not needed anymore
		 ...
		 * returned
		 * @param[in] reverseOrder The optional parameter that indicates that the lookup should run in reverse order, the
		...
		/**
		 * Each layer can print a string representation of the layer most important data using Layer#toString(). This method
		 * aggregates this string from all layers and print it to a complete string containing all packet's relevant data
		 * @param[in] timeAsLocalTime Print time as local time or GMT. Default (true value) is local time, for GMT set to
		 * false
