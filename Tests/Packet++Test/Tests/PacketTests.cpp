...
PTF_TEST_CASE(CopyLayerAndPacketTest)
{
	...
	PTF_ASSERT_TRUE(sampleTcpPacketWithOptions.getLayerOfType<pcpp::TcpLayer>()
						->getTcpOption(pcpp::PCPP_TCPOPT_TIMESTAMP)
						.getRecordBasePtr() != tcpLayer.getTcpOption(pcpp::PCPP_TCPOPT_TIMESTAMP).getRecordBasePtr());
	PTF_ASSERT_TRUE(sampleTcpPacketWithOptions.getLayerOfType<pcpp::TcpLayer>()->getTcpOption(
						pcpp::PCPP_TCPOPT_TIMESTAMP) == tcpLayer.getTcpOption(pcpp::PCPP_TCPOPT_TIMESTAMP));
...
PTF_TEST_CASE(PacketTrailerTest)
{
	...
	pcpp::EthLayer newEthLayer(pcpp::MacAddress("30:46:9a:23:fb:fa"), pcpp::MacAddress("6c:f0:49:b2:de:6e"),
							   PCPP_ETHERTYPE_IP);
	...
	PTF_ASSERT_FALSE(pppoeDiscovery
						 ->addTag(pcpp::PPPoEDiscoveryLayer::PPPoETagBuilder(
							 pcpp::PPPoEDiscoveryLayer::PPPOE_TAG_AC_NAME, 0x42524153))
						 .isNull());
...
PTF_TEST_CASE(PrintPacketAndLayers)
{
	...
	for (std::vector<std::string>::iterator iter = expectedLayerStrings.begin(); iter != expectedLayerStrings.end();
		 iter++)
	{
