...
static struct option BgpAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
...
class GlobalConfig
{
  ...
  public:
	...
	// a flag indicating whether to write BGP data to actual files or to console
	bool writeToConsole;
...
typedef std::map<std::string, BgpReassemblyData> BgpReassemblyMgr;
typedef std::map<std::string, BgpReassemblyData>::iterator BgpReassemblyMgrIter;
...
void printUsage()
{
	std::cout << std::endl
			  ...
			  << "    -e bpf_filter : Apply a BPF filter to capture file or live interface, meaning BGP reassembly "
...
static void OnBgpMessageReadyCallback(pcpp::BgpPacketData *bgpData, void *userCookie)
{
/* 	1. manager 存 BgpReassemblyData   									yes
	2. manager 的指定 BgpReassemblyData 里边fileStream 是否为NULL
		2.1 将当前（指传入的参数）的名称加入opened列表
		2.2 如果打开的文件已达上限，关闭目前的
		2.3 设置文件名
		2.4 打开文件， 模式由之前2.2设置的reopenFileStreams决定
	3. 更改BgpReassemblyData里的统计值
	4. 将数据写入打开的文件里
 */
	...
	BgpReassemblyMgrIter iter = mgr->find(bgpData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(bgpData->getTupleName(), BgpReassemblyData()));
		iter = mgr->find(bgpData->getTupleName());
	}
	...
	if (iter->second.fileStream == NULL)
	{
		...
		if (result == 1)
		{
			BgpReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
...
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *bgpReassemblyCookie)
{
	...
	pcpp::BgpReassembly *bgpReassembly = (pcpp::BgpReassembly *)bgpReassemblyCookie;
	bgpReassembly->reassemblePacket(packet);
}
...
void doBgpReassemblyOnPcapFile(std::string fileName, pcpp::BgpReassembly &bgpReassembly, std::string bpfFilter = "")
{
	...
	// run in a loop that reads one packet from the file in each iteration and feeds it to the BGP reassembly instance
	pcpp::RawPacket rawPacket;
	while (reader->getNextPacket(rawPacket))
	{
		bgpReassembly.reassemblePacket(&rawPacket);
	}
								  ...
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &bgpReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", BgpAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	BgpReassemblyMgr mgr;

	//  create the BGP reassembly instance
	pcpp::BgpReassembly bgpReassembly(OnBgpMessageReadyCallback, &mgr);
	...
	else // analyze in live traffic mode
	{
		...
		// start capturing packets and do BGP reassembly
		doBgpReassemblyOnLiveTraffic(dev,bgpReassembly, bpfFilter);
	}
}