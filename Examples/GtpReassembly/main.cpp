...
static struct option GtpAssemblyOptions[] = {{"interface", required_argument, 0, 'i'},
 ...
 */
	...
	GtpReassemblyMgr *mgr = (GtpReassemblyMgr *)userCookie;

	// check if this tuple already appears in the manager. If not add it
	GtpReassemblyMgrIter iter = mgr->find(gtpData->getTupleName());
	if (iter == mgr->end())
	{
		mgr->insert(std::make_pair(gtpData->getTupleName(), GtpReassemblyData()));
		iter = mgr->find(gtpData->getTupleName());
	}
	...
	if (iter->second.fileStream == NULL)
	{
		...
		int result =
			GlobalConfig::getInstance().getRecentFilesWithActivity()->put(gtpData->getTupleName(), &nameToCloseFile);
		...
		if (result == 1)
		{
			GtpReassemblyMgrIter iter2 = mgr->find(nameToCloseFile);
			if (iter2 != mgr->end())
			{
	...
	iter->second.bytes += (int)gtpData->getDataLength();

	// 4. 

	// write the new data to the file
	iter->second.fileStream->write((char*)gtpData->getData(), gtpData->getDataLength());
 ...
 */
static void onPacketArrives(pcpp::RawPacket *packet, pcpp::PcapLiveDevice *dev, void *gtpReassemblyCookie)
{
	// get a pointer to the TCP reassembly instance and feed the packet arrived to it
	pcpp::GTPReassembly *gtpReassembly = (pcpp::GTPReassembly *)gtpReassemblyCookie;
	gtpReassembly->reassemblePacket(packet);
}
...
void doGtpReassemblyOnPcapFile(std::string fileName, pcpp::GTPReassembly &gtpReassembly, std::string bpfFilter = "")
{
	...
	{
		gtpReassembly.reassemblePacket(&rawPacket);
	}
								  ...
								  std::string bpfFilter = "")
{
	...
	dev->startCapture(onPacketArrives, &gtpReassembly);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "i:r:o:e:f:mcvhl", GtpAssemblyOptions, &optionIndex)) != -1)
	{
		switch (opt)
		{
		case 0:
	...
	GtpReassemblyMgr mgr;

	//  create the TCP reassembly instance
	pcpp::GTPReassembly gtpReassembly(OnGtpMessageReadyCallback, &mgr);

	// analyze in pcap file mode
	if (!inputPcapFileName.empty())
	{
		doGtpReassemblyOnPcapFile(inputPcapFileName, gtpReassembly, bpfFilter);
	}
	else // analyze in live traffic mode
	{
		...
		doGtpReassemblyOnLiveTraffic(dev, gtpReassembly, bpfFilter);
	}
}