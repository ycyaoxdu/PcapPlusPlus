 ...
 * represented by a class. The filter, at the end, is that class.<BR> For example: the filter "src host 1.1.1.1" will be
...
class BpfFilterWrapper
{
  ...
  public:
	...
	/**
	 * Set a filter. This method receives a filter in BPF syntax (https://biot.com/capstats/bpf.html) and an optional
	 * link type, compiles them, and if compilation is successful it stores the filter.
...
class GeneralFilter
{
  ...
  public:
	 ...
	 * @param[out] result An empty string that the parsing will be written into. If the string isn't empty, its content
	 * will be overridden
...
class BPFStringFilter : public GeneralFilter
{
  ...
  public:
	 ...
	 * A method that parses the class instance into BPF string format
	 * @param[out] result An empty string that the parsing will be written into. If the string isn't empty, its content
	 * will be overridden If the filter is not valid the result will be an empty string
	 * @return No return value
	 */
...
class IFilterWithDirection : public GeneralFilter
{
  ...
  protected:
	...
	IFilterWithDirection(Direction dir)
	{
		m_Dir = dir;
	}
 ...
 * An abstract class that is the base class for all filters which contain an operator (e.g X equals Y; A is greater than
 * B; Z1 not equals Z2, etc.). This class cannot be instantiated<BR> For deeper understanding of the filter concept
...
class IFilterWithOperator : public GeneralFilter
{
  ...
  protected:
	...
	IFilterWithOperator(FilterOperator op)
	{
		m_Operator = op;
	}
...
class IPFilter : public IFilterWithDirection
{
  ...
  public:
	 ...
	 * @param[in] ipAddress The IPv4 address to build the filter with. If this address is not a valid IPv4 address an
	 * error will be written to log and parsing this filter will fail
	 * @param[in] dir The address direction to filter (source or destination)
	 */
	IPFilter(const std::string &ipAddress, Direction dir)
		: IFilterWithDirection(dir), m_Address(ipAddress), m_IPv4Mask(""), m_Len(0)
	{
	}

	/**
	 * A constructor that enable to filter only part of the address by using a mask (aka subnet). For example: "filter
	 * only IP addresses that matches the subnet 10.0.0.x"
	 * @param[in] ipAddress The IPv4 address to use. Only the part of the address that is not masked will be matched.
	 * For example: if the address is "1.2.3.4" and the mask is "255.255.255.0" than the part of the address that will
	 * be matched is "1.2.3.X". If this address is not a valid IPv4 address an error will be written to log and parsing
	 * this filter will fail
	 * @param[in] dir The address direction to filter (source or destination)
	 * @param[in] ipv4Mask The mask to use. Mask should also be in a valid IPv4 format (i.e x.x.x.x), otherwise parsing
	 * this filter will fail
	 */
	 ...
	 * matched is "1.2.3.X". If this address is not a valid IPv4 address an error will be written to log and parsing
	 * this filter will fail
	 * @param[in] dir The address direction to filter (source or destination)
	 * @param[in] len The subnet to use (e.g "/24")
	 */
	IPFilter(const std::string &ipAddress, Direction dir, int len)
		: IFilterWithDirection(dir), m_Address(ipAddress), m_IPv4Mask(""), m_Len(len)
	{
	}
 ...
 * protocols, such as ARP, IPv4, IPv6, VLAN tags, etc.<BR> For deeper understanding of the filter concept please refer
 * to PcapFilter.h
 */
...
/**
 * @class AndFilter
 * A class for connecting several filters into one filter with logical "and" between them. For example: if the 2 filters
 * are: "IPv4 address = x.x.x.x" + "TCP port dst = 80", then the new filter will be: "IPv4 address = x.x.x.x _AND_ TCP
 * port dst = 80"<BR> This class follows the composite design pattern<BR> For deeper understanding of the filter concept
 * please refer to PcapFilter.h
...
class AndFilter : public GeneralFilter
{
  ...
  public:
	...
	/**
	 * A constructor that gets a list of pointers to filters and creates one filter from all filters with logical "and"
	 * between them
...
/**
 * @class OrFilter
 * A class for connecting several filters into one filter with logical "or" between them. For example: if the 2 filters
 * are: "IPv4 address = x.x.x.x" + "TCP port dst = 80", then the new filter will be: "IPv4 address = x.x.x.x _OR_ TCP
 * port dst = 80"<BR> This class follows the composite design pattern<BR> For deeper understanding of the filter concept
 * please refer to PcapFilter.h
 * @todo add some methods: "addFilter", "removeFilter", "clearAllFilter"
 */
class OrFilter : public GeneralFilter
{
  ...
  public:
	...
	/**
	 * A constructor that gets a list of pointers to filters and creates one filter from all filters with logical "or"
	 * between them
	 * @param[in] filters The list of pointers to filters
	 */
 ...
 * ::TCP, ::UDP, ::VLAN, ::IPv4, ::IPv6, ::ARP, ::Ethernet, ::GRE (distinguish between ::GREv0 and ::GREv1 is not
 * supported). <BR> For deeper understanding of the filter concept please refer to PcapFilter.h
 */
class ProtoFilter : public GeneralFilter
{
  ...
  public:
	 ...
	 * @param[in] proto The protocol to filter, only packets matching this protocol will be received. Please note not
	 * all protocols are supported. List of supported protocols is found in the class description
	 */
...
/**
 * @class ArpFilter
 * A class for filtering ARP packets according the ARP opcode. When using this filter only ARP packets with the relevant
 * opcode will be received <BR> For deeper understanding of the filter concept please refer to PcapFilter.h
 */
...
/**
 * @class VlanFilter
 * A class for filtering VLAN tagged packets by VLAN ID. When using this filter only packets tagged with VLAN which has
 * the specific VLAN ID will be received <BR> For deeper understanding of the filter concept please refer to
 * PcapFilter.h
 */
...
class TcpFlagsFilter : public GeneralFilter
{
  public:
	...
	/**
	 * An enum for representing 2 type of matches: match only packets that contain all flags defined in the filter or
  ...
  public:
	 ...
	 * @param[in] tcpFlagBitMask A 1-byte bitmask containing all TCP flags participating in the filter. This parameter
	 * can contain the following value for example: TcpFlagsFilter::tcpSyn | TcpFlagsFilter::tcpAck |
...
class TcpWindowSizeFilter : public IFilterWithOperator
{
  ...
  public:
	/**
	 * A constructor that get the window-size and operator and creates the filter. For example: "filter all TCP packets
	 * with window-size less than 1000"
...
class UdpLengthFilter : public IFilterWithOperator
{
  ...
  public:
	/**
	 * A constructor that get the UDP length and operator and creates the filter. For example: "filter all UDP packets
	 * with length greater or equal to 500"
