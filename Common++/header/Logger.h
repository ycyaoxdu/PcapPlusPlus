...
enum LogModule
{
	...
	PacketLogModuleSctpLayer,					   ///< SctpLayer module (Packet++)
 ...
 *
 * PcapPlusPlus is divided into modules (described in #LogModule enum). The user can set the log level got each module
 * or to all modules at once. The default is Logger#Info which outputs only error messages. Changing log level for
 * modules can be done dynamically while the application is running.
 *
 * The logger also exposes a method to retrieve the last error log message.
 *
 * Logs are printed to console by default in a certain format. The user can set a different print function to change the
 * format or to print to other media (such as files, etc.).
 *
...
#define PCPP_LOG_DEBUG(message)                                                                                        \
	...
	{                                                                                                                  \
		...
		{                                                                                                              \
			...
			(*sstream) << message;                                                                                     \
			pcpp::Logger::getInstance().internalPrintLogMessage(sstream, pcpp::Logger::Debug, __FILE__, __FUNCTION__,  \
																__LINE__);                                             \
		}                                                                                                              \
	} while (0)
...
#define PCPP_LOG_ERROR(message)                                                                                        \
	...
	{                                                                                                                  \
		std::ostringstream *sstream = pcpp::Logger::getInstance().internalCreateLogStream();                           \
		(*sstream) << message;                                                                                         \
		pcpp::Logger::getInstance().internalPrintLogMessage(sstream, pcpp::Logger::Error, __FILE__, __FUNCTION__,      \
															__LINE__);                                                 \
	} while (0)
